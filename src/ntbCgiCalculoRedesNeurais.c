...
void adicionarNaListaEncadeada(chainType_2 **first, xmlNodePtr noPai, char *tag, char *nameAtribute)
{
	...
	if((strcmp((char *)new->value,"sim")) == 0)
	{
		new->entry = 1.0;
	}
	else
	{
		if((strcmp((char *)new->value,"nao")) == 0)
			new->entry = -1.0;
		else
			new->entry =  0.0;
	}
...
chainType_2 *converterParaCalculoRedesNeurais(char *pid)
{
	...
	if((noTriagem = pegarFilhoPorNome(noPaciente, "triagem")) == NULL)
	{
		usualFreeMemory(NULL);
		flock(fileno(document), LOCK_EX);
		fclose(document);
...
/*
	...
	while(first_2 != NULL)
	{
		printf("%s - %s = %1.1f<br>",first_2->attribute, first_2->value, first_2->entry);
...
//	printf("- %f -\n",response);
	...
	aux = first;
	if((erro =nnet2(aux, &grupo, &raioOriginal)))
	{
